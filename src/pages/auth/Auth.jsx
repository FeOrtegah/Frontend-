...
const Auth = () => {
  ...
  const handleRegistro = async (e) => {
    ...
    try {
      ...
      await UserService.createUser({
        nombre, email, password, telefono: telefono || "", direccion: direccion || "", rol: "cliente"
      });

      setSuccess("Cuenta creada correctamente. Ahora puedes iniciar sesión");
      setFormData(prev => ({
        ...prev,
        registro: { nombre: "", email: "", password: "", confirmarPassword: "", telefono: "", direccion: "" }
      }));
  ...
  const handleDemoLogin = async () => {
    try {
      ...
      const demoUser = { email: "demo@efa.com", password: "demo123" };
            ...
            <Card.Body>
                ...
                <Tab eventKey="login" title="Iniciar Sesión">
                  ...
                  <Form onSubmit={handleLogin}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control type="email" value={formData.login.email} required placeholder="tu@email.com"
                        onChange={(e) => handleInputChange("login", "email", e.target.value)} />
                    </Form.Group>

                    <Form.Group className="mb-4">
                      <Form.Label>Contraseña</Form.Label>
                      <Form.Control type="password" value={formData.login.password} required placeholder="Tu contraseña"
                        onChange={(e) => handleInputChange("login", "password", e.target.value)} />
                    </Form.Group>

                    <Button variant="primary" type="submit" className="w-100 py-2" disabled={loading}>
                      {loading ? <><Spinner animation="border" size="sm" className="me-2" />Iniciando Sesión...</> : "Iniciar Sesión"}
                    </Button>
                  </Form>

                  <div className="text-center mt-3">
                    <Button variant="outline-secondary" size="sm" onClick={handleDemoLogin} disabled={loading}>
                      Acceso Demo
                    </Button>
                  </div>
                </Tab>
                ...
                <Tab eventKey="registro" title="Crear Cuenta">
                  ...
                  <Form onSubmit={handleRegistro}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control type="text" value={formData.registro.nombre} required placeholder="Tu nombre completo"
                        onChange={(e) => handleInputChange("registro", "nombre", e.target.value)} />
                    </Form.Group>

                    <Form.Group className="mb-3">
                      <Form.Label>Correo electrónico</Form.Label>
                      <Form.Control type="email" value={formData.registro.email} required placeholder="tu@email.com"
                    ...
                    <Row>
                      <Col md={6}>
                        <Form.Group className="mb-3">
                          <Form.Label>Contraseña</Form.Label>
                          <Form.Control type="password" value={formData.registro.password} required placeholder="Mínimo 6 caracteres" minLength="6"
                      ...
                      <Col md={6}>
                        <Form.Group className="mb-3">
                          ...
                          <Form.Control type="password" value={formData.registro.confirmarPassword} required placeholder="Repite tu contraseña"
                    ...
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control type="tel" value={formData.registro.telefono} placeholder="+56 9 1234 5678"
                        onChange={(e) => handleInputChange("registro", "telefono", e.target.value)} />
                    </Form.Group>

                    <Form.Group className="mb-4">
                      <Form.Label>Dirección (Opcional)</Form.Label>
                      <Form.Control type="text" value={formData.registro.direccion} placeholder="Tu dirección completa"
                    ...
                    <Button variant="success" type="submit" className="w-100 py-2" disabled={loading}>
                      {loading ? <><Spinner animation="border" size="sm" className="me-2" />Creando Cuenta...</> : "Crear Cuenta"}
                    </Button>
                  </Form>
                </Tab>
              </Tabs>
              ...
              <div className="text-center mt-3">
                <small className="text-muted">
                  Al registrarte, aceptas nuestros <a href="/terminos" className="text-decoration-none">Términos de Servicio</a> y <a href="/privacidad" className="text-decoration-none">Política de Privacidad</a>
                </small>
              </div>
            </Card.Body>
          </Card>
